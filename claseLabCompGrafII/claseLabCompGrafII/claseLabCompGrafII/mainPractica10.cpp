...
void InitGL ( GLvoid )     // Inicializamos parametros
{
	...
	//Carga de Figuras
	kit._3dsLoad((char *)"kitt.3ds");
	//kit.VertexNormals();
	
	llanta._3dsLoad((char *)"k_rueda.3ds");
...
void display ( void )   // Creamos la funcion donde se dibuja
{
	...
	glPushMatrix();
		...
		glPushMatrix();		
			...
			glPushMatrix();
				...
				glRotatef(rotCarro, 0.0, 1.0, 0.0); // Rotacion de Carro

				//Pongo aquí la carroceria del carro
				kit.GLrender(NULL,_SHADED,1.0);  //_WIRED O _POINTS

				glPushMatrix(); // Delantera Derecha
					glTranslatef(-6.0f,-1.0f,7.5f);
					glRotatef(animLlantas,1.0,0.0,0.0);
					llanta.GLrender(NULL,_SHADED,1.0);
				glPopMatrix();

				glPushMatrix(); // Trasera Derecha
					glTranslatef(-6.0f, -1.0f, -9.5f);
					glRotatef(animLlantas, 1.0, 0.0, 0.0);
					llanta.GLrender(NULL, _SHADED, 1.0);
				glPopMatrix();
	...
	glDisable(GL_LIGHTING);
		...
		pintaTexto(-12,12.0,-14.0,(void *)font, (char *)"Practica 10");
		pintaTexto(-12,10.5,-14,(void *)font, (char *)"Poner algo en Movimiento");
		glColor3f(1.0,1.0,1.0);
	glEnable(GL_LIGHTING);
	glEnable(GL_TEXTURE_2D);
